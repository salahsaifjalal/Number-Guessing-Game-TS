...
let gameStartingView = function () {
  let gameAnimation = myChalkAnimation.rainbow(
    makeBox(`   Welcome to CLI based Number Guessing Game!    `, {
...
let askQuestion = async function () {
  ...
  while (true) {
    if (noOfAttempt !== 0) {
        ...
        if (userInput.userNumber === computerGeneratedNumber) {
          console.log(
            chalk.greenBright.bold(
              ` Congratulations! You guessed the correct number! It was ${computerGeneratedNumber} `
            )
          );
          sysSays.speak(
            `Congratulations! You got it.    It was ${computerGeneratedNumber}`
          );
      ...
      console.log(
        chalk.bgRed(`Game Over... The number was ${computerGeneratedNumber} `)
      );
      break;
    }
  }
...
let restartGame = async function () {
  ...
  if (nowrestartGame.userDecision === endingChoices[0]) {
    ...
    const endingAnimation = myChalkAnimation.neon(
